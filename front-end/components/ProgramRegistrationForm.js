...
const ProgramRegistrationForm = () => {
  ...
  const programTitleEl = useRef(null)
  const programFieldEl = useRef(null)
  ...
  const registerProgram = async (e) => {
    ...
    await contract.methods
      .registerProgram(programTitleEl.current.value, programFieldEl.current.value)
      .send({
        from: account,
      })
      ...
      .then((res) => {
        ...
        setTxCompleted(true)
        setTxMessage(
          ` ${txMessage} The transaction is now on chain and mined in block ${res.blockNumber}. Successful! 🎉 Program registered.`
        )
      })
  }
  ...
  const reset = async (e) => {
    ...
    programTitleEl.current.value = ''
    programFieldEl.current.value = ''
  ...
  return (
    <div className="flex w-full flex-col items-center justify-center">
      ...
      <form
        ...
        onSubmit={registerProgram}
      >
        <div className="w-full">
          <label className="sr-only" htmlFor="program-title-input">
            Program Title
          </label>
          <input
            autoComplete="off"
        ...
        <div className="w-full">
          <label className="sr-only" htmlFor="program-field-input">
            Program Field
          </label>
          <input
            autoComplete="off"
            className="w-full rounded-md px-4 focus:border-transparent focus:outline-none focus:ring-2 focus:ring-primary-600 dark:bg-black"
            id="program-field-input"
            name="program-field"
