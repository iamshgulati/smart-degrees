...
const StudentAcceptanceForm = () => {
  const { account, contract, contractAddress } = useLayoutContext()

  const studentAddressEl = useRef(null)
  ...
  const acceptStudent = async (e) => {
    ...
    await contract.methods
      .acceptStudent(studentAddressEl.current.value)
      .send({
        from: account,
      })
      ...
      .then((res) => {
        ...
        setTxCompleted(true)
        setTxMessage(
          ` ${txMessage} The transaction is now on chain and mined in block ${res.blockNumber}. Successful! 🎉 Student accepted.`
        )
      })
  }
  ...
  const reset = async (e) => {
    ...
    studentAddressEl.current.value = ''
  ...
  return (
    <div className="flex w-full flex-col items-center justify-center">
      ...
      <form
        ...
        onSubmit={acceptStudent}
      >
        <div className="w-full">
          <label className="sr-only" htmlFor="student-address-input">
            Student Address
          </label>
          <input
            autoComplete="off"
...
export default StudentAcceptanceForm
